...
from .base_model import BaseLLMModel, CallbackToIterator, NanhuiCallbackHandler
from ..config import default_Nanhui_assistant_model
...
class NanhuiAgent_Client(BaseLLMModel):
    def __init__(self, model_name, openai_api_key, user_name="") -> None:
        ...
        self.llm = ChatOpenAI(openai_api_key=openai_api_key, temperature=0, model_name=default_Nanhui_assistant_model, openai_api_base=os.environ.get("OPENAI_API_BASE", None))
    ...
    def get_answer_stream_iter(self):
        ...
        manager = BaseCallbackManager(handlers=[NanhuiCallbackHandler(it.callback)])
        def thread_func():
            tools = self.tools
            if self.index is not None:
                    tools.append(
